...
interface
uses System.JSON, uEnumrationsList, Rtti, SysUtils;
type
  TCEPGenerics = class
  private
    FsCep: string;
...
begin
  ...
  begin
    with oJsonObject do
      if ((UpperCase(oRttiProperty.Name) <> 'JSON') and (UpperCase(oRttiProperty.Name) <> 'SITETYPE')) then
      begin
        AddPair(lowerCase(oRttiProperty.Name), oRttiProperty.GetValue(Pointer(Self)).ToString);
      end;
  end;
